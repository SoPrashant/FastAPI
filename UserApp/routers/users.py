...
async def read_user(user: user_dependency, db : db_dependency, user_id : int = Path(gt=0)):
    ...
    user_model = db.query(Users).filter(Users.id == user_id).filter(Users.owner_id == user.get('id')).first()
...
async def update_user(
    ...
    user_id: int = Path(gt=0)
        ...
        user_model = db.query(Users).filter(Users.id == user_id)\
            .filter(Users.owner_id == user.get('id')).first()
...
async def update_user(user : user_dependency, db : db_dependency, user_request: UserRequest, user_id : int = Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=401, detail="Authentication Failed")
...
async def delete_user(user : user_dependency, db: db_dependency, user_id: int= Path(gt=0)):
    ...
    db.query(Users).filter(Users.id == user_id).filter(Users.owner_id == user.get('id')).delete()
    db.commit()
